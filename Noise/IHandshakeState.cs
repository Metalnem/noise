...
namespace Noise
{
	...
	public interface IHandshakeState : IDisposable
	{
		...
		/// </summary>
		(int, ITransport) WriteMessage(ReadOnlySpan<byte> payload, Span<byte> messageBuffer);
		...
		/// </summary>
		(int, ITransport) ReadMessage(ReadOnlySpan<byte> message, Span<byte> payloadBuffer);
